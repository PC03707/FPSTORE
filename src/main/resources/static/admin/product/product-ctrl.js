app.controller("product-ctrl", function($scope, $http) {
	...
	$scope.bannernav = function(files) {
		var item = angular.copy($scope.banner_nav);
		
		var data = new FormData();
		
		data.append('file', files[0]);
		
		$scope.banner_nav.image = files[0].name;
		
		console.log($scope.banner_nav.image);
		
		$http.put(`/rest/products/changepanels/${item.linkimage}`, $scope.banner_nav).then(resp => {
	...
	$scope.navChange = function(files) {
		var item = angular.copy($scope.navChange);
		var data = new FormData();
		data.append('file', files[0]);
